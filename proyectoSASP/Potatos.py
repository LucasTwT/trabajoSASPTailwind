...
def main() -> None:
    ...
    train_img_folder = ImageFolder(
						r'proyectoFlask\Data\Potatos',
                            transforms=transform)
    #dataloaders:
    train_dataloader = DataLoader(train_img_folder, batch_size=32, shuffle=True)
    
    model = configuracion_modelo(models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1), 2)
    ...
    with open(r'proyectoFlask\out\resTrainPotatos.json', 'w') as f:
        json.dump(error, f)
    ...
    if opc:
        ruta_guardado = r'proyectoFlask\out\pesos\Potatos.pth'
